...
{
	FILE* fsamprom = fopen("/projects/musix/assets/kurzweil/k2v3internalsamplerom.bin","rb");
...
{	
	...
	//afInitCompression(af_setup, AF_DEFAULT_TRACK, AF_COMPRESSION_G711_ALAW);
	...
	if( false ) for( int i=0; i<inumframes; i++ )
	{
	...
	auto af_written = afWriteFrames(af_file, AF_DEFAULT_TRACK, buffer,
		inumframes);
